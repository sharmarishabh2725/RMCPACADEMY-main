          ...
          >
            <div className="bg-white p-8 sm:p-10 rounded-3xl border border-brand-blue/10 shadow-2xl relative">
              ...
              <div className="space-y-5 text-slate-600 text-base sm:text-lg leading-relaxed">
                {cmsData?.principal_message ? (
                  <p className="whitespace-pre-wrap">{cmsData.principal_message}</p>
                ) : (
                  <>
                    ...
                    <p>
                      ...
                      , a place where learning is a journey, not just a destination. It is our mission to provide an education that fosters both intellectual growth and personal character development.
                    </p>
                    <p>
                      At RMCP Academy, we are committed to excellence in academics, innovation in teaching, and the holistic development of our students. We believe in cultivating critical thinking, creativity, and leadership skills to prepare our students for a dynamic, changing future.
                    </p>
                    <p>
                      Our passionate educators, state-of-the-art facilities, and enriching extracurricular programs ensure that every child receives the best possible learning experience. We encourage students to dream big, challenge themselves, and embrace learning with enthusiasm.
                    </p>
                    <p>
                      I invite parents and guardians to be active partners in this journey. Together, we can create a vibrant, nurturing environment where our students thrive and become responsible, compassionate individuals.
                    </p>
                  </>
                )}
              </div>
              ...
              <div className="mt-10 pt-6 border-t border-brand-blue/10 flex flex-col items-end text-right">
                ...
                <p className="text-brand-text text-lg font-black font-heading mt-1">
                  {cmsData?.principal_name || "MR. ANANT SAXENA"}
                </p>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">{cmsData?.principal_title || "Principal, RMCP Academy"}</p>
              </div>

            </div>
          </motion.div>

          {/* Principal Image Column (Right) */}
          <motion.div 
            initial={{ opacity: 0, x: 35 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            ...
            <div className="flex flex-col items-center">
              <div className="relative p-3 bg-white border border-brand-blue/10 rounded-3xl shadow-2xl max-w-sm w-full group">
                ...
                <div className="relative rounded-2xl overflow-hidden aspect-[4/5] bg-brand-surface">
                  <img
                    src={cmsData?.principal_image_url || principleImag}
                    alt="Principal Mrs. Shikha Saxena"
              ...
              <div className="text-center bg-brand-text p-6 rounded-2xl shadow-2xl -mt-10 w-5/6 relative z-20 border border-white/5">
                <h3 className="text-lg font-black text-white font-heading tracking-wide">
                  {cmsData?.principal_name || "Mrs. Shikha Saxena"}
                </h3>
                <p className="text-brand-orange font-bold text-xs uppercase tracking-widest mt-1.5">{cmsData?.principal_title || "Principal"}</p>
                <div className="w-12 h-0.5 bg-brand-blue mx-auto my-3 rounded"></div>
                <p className="text-slate-355 text-xs leading-relaxed font-medium">
                  Visionary Educator & Administrative Leader
                </p>
              </div>
            </div>
          </motion.div>
